...
public class server_set
{
    ...
    {
        ServerSocket socket=new ServerSocket(2020); //server_set Socket running at port 2020
        int i=0;
        System.out.println("server_set is waiting for Connections");
        while(true){
...
class Client_Handler implements Runnable{
    ...
    void CloseStreams() throws IOException {
        ...
        input.close();
        connection.close();
        server_set.sockets.remove(this);
        System.out.println("Current Client Count- "+ server_set.sockets.size());
    }
        ...
        try{
            while(true) {
                ...
                if(msg.endsWith("Client_wants_to_end_the_connection")) {
                    msg=msg.substring(0,msg.indexOf('-'))+" has left the server_set";
                    for(Client_Handler c:server_set.sockets) {
